...
for line in newInput:
    #woorden=str(re.findall(r"(\w+)",line))
    #print(woorden)
    byr=re.findall('byr', line)
    iyr=re.findall('iyr', line)
    eyr=re.findall('eyr', line)
    hgt=re.findall('hgt', line)
    hcl=re.findall('hcl', line)
    ecl=re.findall('ecl', line)
    pid=re.findall('pid', line)
    aantal=len(byr)+len(iyr)+len(eyr)+len(hgt)+len(hcl)+len(ecl)+len(pid)
    #print(aantal)
    if aantal==7:
        valid.append(line)
#print(valid)
...
for passport in valid:
    condities=re.findall('(\w+:[^\s]+)', passport)
    #print(condities)
    allescorrect=True
    for element in condities:
        splittedelement=element.split(':')
        #print(splittedelement)
        #allescorrect=True
        if splittedelement[0]=='byr':
            if not (1920 <= int(splittedelement[1]) <= 2002):
                allescorrect=False
        if splittedelement[0]=='iyr':
            if not (2010 <= int(splittedelement[1]) <=2020):
                allescorrect=False
        if splittedelement[0]=='eyr':
            if not (2020 <= int(splittedelement[1]) <= 2030):
                allescorrect=False
        if splittedelement[0]=='hgt':
            cm=re.findall('cm', splittedelement[1])
            inch=re.findall('in', splittedelement[1])
            height=re.findall('\d+', splittedelement[1])
            if not len(re.findall('^\d+(cm|in)$', splittedelement[1]))==1:
                allescorrect=False
            if len(cm)==1:
            ...
            if len(inch)==1:
                if not (59 <= int(height[0]) <= 76):
                    allescorrect=False
        if splittedelement[0]=='hcl':
            #print(splittedelement[1])
            hair=re.findall('#[0-9a-f]{6}$', splittedelement[1])
            #print(hair)
            if not len(hair)==1:
                allescorrect=False
        if splittedelement[0]=='ecl':
            oog=re.findall('^(amb|blu|brn|gry|grn|hzl|oth)$', splittedelement[1])
            if not len(oog)==1:
                allescorrect=False
        if splittedelement[0]=='pid':
            id=re.findall('^\d{9}$',splittedelement[1])
            if not len(id)==1:
                allescorrect=False
    if allescorrect==True:
