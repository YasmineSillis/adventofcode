...
beweeg = function(richting, positie) {
  nieuwePositie = c(positie[1], positie[2])
  if (richting == "v") {
    nieuwePositie[2] = nieuwePositie[2] - 1
  }
  if (richting == "^") {
    nieuwePositie[2] = nieuwePositie[2] + 1
  }
  if (richting == "<") {
    nieuwePositie[1] = nieuwePositie[1] - 1
  }
  if (richting == ">") {
    nieuwePositie[1] = nieuwePositie[1] + 1
  }
...
huidigeLetterPositie = 1
letters = strsplit(input, "")[[1]]
alleCoordinaten = matrix(nrow = nchar(input), ncol = 2)
repeat {
  letter = letters[huidigeLetterPositie]

  if (isRobot) {
    positieRobot = beweeg(richting = letter, positie = positieRobot)
    alleCoordinaten[huidigeLetterPositie,] = positieRobot
  }
  else {
    positieSanta = beweeg(richting = letter, positie = positieSanta)
    alleCoordinaten[huidigeLetterPositie,] = positieSanta

  }
  isRobot = !isRobot
  huidigeLetterPositie = huidigeLetterPositie + 1
  if (huidigeLetterPositie > nchar(input)) {
    break;
  }
}

dim(unique(alleCoordinaten))
